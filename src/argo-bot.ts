...
const BotActions = Object.freeze({
  Unlock: "unlock",
  Diff: "diff",
  Sync: "sync",
  Preview: "preview",
...
const BotDiffActions = Object.freeze({
  AutoSync: "--auto-sync",
  All: "--all",
  Dir: "--dir",
  DirShort: "-d",
...
const BotHelp = Object.freeze({
  Diff: diffHelp,
  History:
    "[Not yet supported] history [app name], prints deployment history of app",
  Info: "usage: 'info [app name]`, view info for a specific app",
  Preview: "[Not yet supported] deploy temporary PR",
  Rollback:
    "rollback [optional: --dry-run or -d] [app name], use 'rollback --dry-run app' or 'rollback -d app' to see latest deployment info, use 'rollback app' to rollback the latest deployment to the previous one, verify rollback using diff command",
  Sync:
    "usage: `sync [app name]`, syncs deployment in GKE cluster with manifests or helm charts using branch in current PR",
...
export class ArgoBot {
  ...
  private static async setPrStatusCheck(
    context,
    stateString,
    descriptionString,
    contextString
    ...
    const branchContext = await ArgoBot.getCurrentBranchContext(
      context,
      prNumber
    );
    await context.github.repos.createStatus({
      owner: branchContext.head.repo.owner.login,
      repo: branchContext.head.repo.name,
      sha: branchContext.head.sha,
      state: stateString,
  ...
  private static async setDiffStatusCheck(context, state) {
    ...
    await ArgoBot.setPrStatusCheck(
      context,
      state,
      "argo diff status",
      "argo/diff"
  ...
  constructor(appContext) {
    ...
    this.argoAPI = new ArgoAPI(
      this.appContext,
      this.argoConfig.getAPIToken(),
      this.argoConfig.getServerIP()
    );
  }
  ...
  public async handleCommand(command) {
    ...
    if (action === BotActions.Diff) {
      ...
      if (arr[2] && arr[2] === BotDiffActions.AutoSync) {
        this.appContext.log(
          "Received diff command with" + BotDiffActions.AutoSync
        );
        jsonResponse = await this.argoAPI.fetchAllAutoSyncApps();
        return await this.handleDiff(jsonResponse);
      } else if (
        arr[2] &&
        (arr[2] === BotDiffActions.Dir || arr[2] === BotDiffActions.DirShort) &&
        arr[3]
      ) {
        ...
        arr[2] &&
        (arr[2] === BotDiffActions.App || arr[2] === BotDiffActions.AppShort) &&
        arr[3]
      ) {
        ...
        jsonResponse = {
          items: [
            { metadata: { name: arr[3] }, spec: { source: { path: appDir } } },
          ],
        };
        return await this.handleDiff(jsonResponse);
      } else if (arr[2]) {
...
`;
    ...
    await ArgoBot.respondWithComment(
      this.appContext,
      "Invalid command; received command: `" + command + "`"
    );
...
`;
  ...
  private async attemptLock() {
    ...
    if (prLock.tryLock(prTitle, prNumber, projectName) === false) {
      const lockMessage =
        prLock.getLockInfo() +
        "; is holding the lock, please merge PR or comment with `" +
        BotCommand +
  ...
  private async handleUnlockAction() {
    ...
    if (prLock.getPrNumber() === prNumber) {
      ...
      return await ArgoBot.respondWithComment(
        this.appContext,
      ...
      const lockMessage =
        prLock.getLockInfo() +
        "; is holding the lock, please comment on that PR to unlock";
...
`;
  ...
  private async handleSync(appName) {
    ...
    const syncCommand =
      "./src/sh/sync_current_branch.sh " + appName + " " + curBranch;
...
`;
  ...
  private async handleDiff(jsonArgoCDApps) {
    const cloneUrl =
      "https://" +
      this.argoConfig.getGithubToken() +
      ":x-oauth-basic@" +
      "github.com/" +
      this.appContext.payload.repository.full_name;
    const prNumber = this.appContext.payload.issue.number;
    const curBranch = await ArgoBot.getCurrentBranch(this.appContext, prNumber);
    const repoDir = "cloned_repos/pr_" + prNumber;

    const cloneCommand =
      "./src/sh/clone_repo.sh " + repoDir + " " + cloneUrl + " " + curBranch;
    ...
    if (err) {
      const errString = this.buildErrString(
        "git clone and checkout of " + curBranch,
        err.stderr
      );
    ...
    if (Object.entries(jsonArgoCDApps).length === 0) {
      ...
      return await this.respondWithError(
        "Empty JSON reponse, make sure the argocd API is reachable and the JWT token is valid"
      );
    }
    ...
    if (
      jsonArgoCDApps["items"] == null ||
      Object.entries(jsonArgoCDApps["items"]).length === 0
    ) {
      await ArgoBot.setDiffStatusCheck(this.appContext, "failure");
      return await this.respondWithError(
        "No Kubernetes deployments found, try running `argo diff --all`"
      );
    }
    ...
    for (const key of Object.keys(jsonItems)) {
      ...
      const command =
        "./src/sh/diff_repo.sh " + repoDir + " " + appName + " " + gitPath;
      const failingExitCodeForDiff = 2;
      this.appContext.log("exec-ing: " + command);
      let diffErr, res;
      [diffErr, res] = await to(
        this.execCommand(command, failingExitCodeForDiff)
      );
      if (diffErr) {
