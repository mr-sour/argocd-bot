...
describe("argo-cd-bot", () => {
  ...
  test("diff comment posted on PR, one app in argocd server", async () => {
    ...
    nock("https://api.github.com")
      ...
      .reply(200, { data: { number: 109, head: { ref: branch } } });
    ...
    nock("http://" + argoCDServer)
      .get(
        "/api/v1/applications?fields=items.metadata.name,items.spec.source.path,items.spec.source.repoURL"
      )
      .reply(200, {
        items: [
          { metadata: { name: appName }, spec: { source: { path: appDir } } },
        ],
      });
    // regex match post body should match diff produced by API
    nock("https://api.github.com")
      .post("/repos/robotland/test/issues/109/comments", /===== App Diff ====/)
      .reply(200);
    nock("https://api.github.com")
      .post(
        "/repos/robotland/test/issues/109/comments",
        /If Auto-sync is enabled just merge this PR to deploy the above changes/
      )
      .reply(200);
  ...
  test("diff comment posted on PR, one app in argocd server error in diff", async () => {
    ...
    nock("https://api.github.com")
      .post("/repos/robotland/test/issues/109/comments", /returned an error/)
      .reply(200);
    await probot.receive({ name: "issue_comment", payload: payloadPr1 });
  });
  ...
  test("diff comment posted on PR with non-existent --dir", async () => {
    ...
    nock("http://" + argoCDServer)
      ...
      .reply(200, {
        items: [
          { metadata: { name: appName }, spec: { source: { path: appDir } } },
          {
            metadata: { name: appName + "2" },
            spec: { source: { path: appDir + "2" } },
          },
        ],
      });
    execStub.onCall(1).yields(false, appDiff);
    ...
    nock("https://api.github.com")
      .post(
        "/repos/robotland/test/issues/109/comments",
        /No Kubernetes deployments found, try running/
      )
      .reply(200);
  ...
  test("diff --auto-sync comment posted on PR", async () => {
    ...
    nock("http://" + argoCDServer)
      .get(
        "/api/v1/applications?fields=items.metadata.name,items.spec.source.path,items.spec.source.repoURL,items.spec.syncPolicy.automated"
      )
      .reply(200, {
        items: [
          { metadata: { name: appName }, spec: { source: { path: appDir } } },
          {
            ...
            spec: { source: { path: appDir }, syncPolicy: { automated: {} } },
          },
        ],
      });
